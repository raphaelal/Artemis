...
@Component({
    ...
    templateUrl: './complaint-student-view.component.html',
})
export class ComplaintStudentViewComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (
            (this.isExamMode && !this.timeOfComplaintValid && !this.complaint) ||
    ...
    private isTimeOfComplaintValid(): boolean {
        ...
        } else if (this.result?.completionDate) {
     ...
     */
    private isCompletionDateWithinCourseReviewPeriod(completionDate: Moment): boolean {
        if (!this.exercise.assessmentDueDate || completionDate.isAfter(this.exercise.assessmentDueDate)) {
            return completionDate.isAfter(moment().subtract(this.course?.maxRequestMoreFeedbackTimeDays, 'day'));
        }
        return this.exercise.assessmentDueDate.isAfter(moment().subtract(this.course?.maxRequestMoreFeedbackTimeDays, 'day'));
    }
